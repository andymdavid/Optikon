...
export function renderHomePage({ showArchive, session, filterTags = [], todos = [] }) {
...
function renderSessionSeed(session) {
    return `<script>
    window.__NOSTR_SESSION__ = ${JSON.stringify(session ?? null)};
  </script>`;
}
function buildPageState(todos, filterTags, showArchive, session) {
...
function filterTodos(allTodos, filterTags) {
...
function renderTagFilterBar(allTodos, activeTags, showArchive) {
...
function toggleTag(activeTags, tag, isActive) {
    if (isActive)
        return activeTags.filter((t) => t.toLowerCase() !== tag.toLowerCase());
    return [...activeTags, tag];
}
function collectTags(todos) {
...
function formatAvatarFallback(npub) {
    if (!npub)
        return "•••";
...
function renderLifecycleActions(todo) {
    const transitions = ALLOWED_STATE_TRANSITIONS[todo.state] ?? [];
    const transitionForms = transitions.map((next) => renderStateActionForm(todo.id, next, formatTransitionLabel(todo.state, next)));
    return `
...
function formatTransitionLabel(current, next) {
    if (current === "done" && next === "ready")
        return "Reopen";
...
function renderStateActionForm(id, nextState, label) {
    return `
