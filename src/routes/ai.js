...
import { createTodosFromTasks, latestSummaries, listOwnerScheduled, listOwnerUnscheduled, normalizeSummaryText, persistSummary, } from "../services/todos";
import { isValidDateString, MAX_TASKS_PER_REQUEST } from "../validation";
export function handleAiTasks(url, match) {
    const owner = url.searchParams.get("owner");
    if (!owner)
        return jsonResponse({ message: "Missing owner." }, 400);
    const days = Number(match[1]);
    if (!Number.isFinite(days) || days <= 0)
        return jsonResponse({ message: "Invalid day range." }, 400);
...
export async function handleAiTasksPost(req) {
...
export async function handleSummaryPost(req) {
    const body = (await safeJson(req));
    ...
    if (!summary)
        return jsonResponse({ message: "Unable to save summary." }, 500);
    return jsonResponse({
        owner: summary.owner,
...
export function handleLatestSummary(url) {
    const owner = url.searchParams.get("owner");
    if (!owner)
        return jsonResponse({ message: "Missing owner." }, 400);
